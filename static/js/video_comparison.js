...
function playVids(videoId) {
    ...
    if (vid.readyState > 3) {
        // 移除这里的 vid.play()，因为视频已经在 resizeAndPlay 函数中播放了
        // vid.play(); // 这行会导致重复触发 onplay 事件

        // 检查Canvas是否已经初始化过，避免重复绑定事件和启动渲染循环
        if (videoMerge.dataset.canvasInitialized === 'true') {
            console.log('Canvas already initialized for:', videoId);
            return;
        }
        ...
        // 标记Canvas已初始化
        videoMerge.dataset.canvasInitialized = 'true';
        console.log('Canvas initialized for:', videoId);
    } 
}
...
function resizeAndPlay_slow(element)
{
  console.log('resizeAndPlay_slow', element)
  
  // 检查是否已经初始化过，避免重复初始化
  if (element.dataset.initialized === 'true') {
    console.log('Video already initialized, skipping:', element.id);
    return;
  }
  ...
  console.log('Video initialized:', element.id);
}
